    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 bg-white border-b border-gray-200">
                ...
                <form method="POST" action="{{ route('parent.children.store') }}" id="addChildForm">
                    @csrf
                    
                    <!-- Name -->
                    <div class="mb-6">
                        <label for="name" class="block text-sm font-medium text-gray-700 mb-1">Child's Name</label>
                        <input type="text" name="name" id="name" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                        @error('name')
                            ...
                            @foreach ($subjects as $index => $subject)
                                <div class="flex flex-col md:flex-row md:items-center space-y-2 md:space-y-0 md:space-x-4">
                                    <div class="flex items-center">
                                        <input type="checkbox" name="subjects[]" id="subject_{{ $index }}" value="{{ $subject }}" class="subject-checkbox h-4 w-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500">
                                        <label for="subject_{{ $index }}" class="ml-2 block text-sm text-gray-700">{{ $subject }}</label>
                                    </div>
                                    
                                    <div class="exam-board-container hidden md:ml-6">
                                        <label for="exam_board_{{ $index }}" class="block text-sm text-gray-700">Exam Board</label>
                                        <select name="exam_boards[]" id="exam_board_{{ $index }}" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" disabled>
                    ...
                    <div class="flex justify-end">
                        <button type="submit" id="saveButton" class="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500" disabled>
                            Save
                        </button>
                    </div>
                </form>
